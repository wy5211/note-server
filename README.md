...
- [x] **Phase 6** 分布式事务：本地消息表 vs 事务消息 + 顺序消息（测试 36/36 绿）
- [ ] Phase 7 (可选) Elasticsearch + ShardingSphere

## Phase 6 学到了什么（分布式事务收官）

**场景**：笔记审核通过 → 作者 +50 积分。审核（上游）和积分（下游）隔着 MQ，
本地事务管不过去 —— `note.reward.mode` 一键切换两种业界标准方案：

| | 方案一 `local_msg` 本地消息表 | 方案二 `tx_msg` 事务消息 |
|---|---|---|
| 原子性来源 | 业务行和消息行**同库同事务**（`Propagation.MANDATORY` 把铁律写进代码） | **半消息协议**：先发不可见消息 → 本地事务 → COMMIT/ROLLBACK |
| 投递者 | 自建中继 Job（扫表+指数退避+死信状态） | broker 直接转正投递 |
| 崩溃兜底 | 消息躺在表里，Job 永远会再扫 | broker 每分钟**回查**（checkLocalTransaction 查库说话，默认 15 次） |
| 依赖 | 只要数据库（最朴素的可靠） | 依赖 MQ 的事务协议 |
| 共同终点 | **至少一次投递 + 下游幂等（锁 + 流水查重）= 最终一致** | 同左 |

**顺序消息**（配菜）：评论楼层分配 —— `syncSendOrderly(hashKey=noteId)` 让同一笔记的
评论进同一队列，`ConsumeMode.ORDERLY` 单队列串行消费，`floor = max+1` 的「读-算-写」
才不会被并发打出重复楼层。业务上要什么序，就拿什么当 hashKey。

## Phase 6 踩坑档案

18. **事务消息回调不走普通转换器**：`executeLocalTransaction` 里 `msg.getPayload()`
    拿到的不是发送时的类型（实测 byte[]）—— 对象强转当场炸且无日志。消息契约统一为
    最朴素的 String，两端零歧义
19. **泛型方法调用的显式类型参数要点出来**：`Wrappers.<Comment>lambdaQuery()` 少写那个点，
    javac 解析成「变量 < 变量 > 方法调用」的数学表达式，报「找不到符号: 变量 Wrappers」
    这种迷惑性极强的错
20. **双 Spring 上下文的消费者分摊**：fork 上下文（@TestPropertySource）与主上下文的
    同名消费组分摊队列 —— 测试断言业务结果（积分到账）而非路径，轮询里同时驱动中继兜底
21. **同秒数据的排序抖动**：`orderByDesc(createdAt)` 秒级精度，同秒写入顺序不稳定 ——
    分页断言页宽 10→50，测试不再和排序运气较劲
